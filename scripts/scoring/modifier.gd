...
func _init(p_id: String, p_name: String, p_effect: Effect,
		p_value: float, p_condition: String, p_rarity: String) -> void:
	id = p_id
	name = p_name
	effect = p_effect
	value = p_value
...
static func effect_from_string(s: String) -> int:
	match s:
		"bonus": return Effect.BONUS
		"add_mult": return Effect.ADD_MULT
...
static func effect_to_string(e: Effect) -> String:
	match e:
		Effect.BONUS: return "bonus"
		Effect.ADD_MULT: return "add_mult"
		Effect.X_MULT: return "x_mult"
		Effect.ADD_REROLLS: return "add_rerolls"
	return ""
