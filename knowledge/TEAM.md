# Team

## Members

| Name             | Roles                                                      |
|------------------|------------------------------------------------------------|
| Artem Abaturov   | Game design, Programming, Project Management               |
| Sofia Petrenko   | Art & Visual Design, Programming, Audio, QA & Playtesting  |

## Responsibilities to Distribute

### Game Design
- Core mechanics design (dice systems, combo scoring, synergies)
- Dice/face/modifier design (stats, effects, rarities)
- Balance and tuning (difficulty curve, economy, expected values)
- Round progression design (shop scaling, blind escalation)

### Programming
- Game logic (dice rolling, rerolls, combo resolution, score calculation)
- UI implementation (shop, dice bag, face swapping, combat HUD)
- Animations and VFX (dice rolling, score tallying, transitions)
- Data architecture (face definitions, save/load, configuration)
- Audio integration (hooking up sounds to game events)

### Art & Visual Design
- UI/UX design (layouts, wireframes, interaction flows)
- Sprite art (dice, face icons, modifier cards, shop environment)
- Visual effects (particle effects, highlights, feedback cues)
- Fonts and color palette

### Audio
- Sound effects (dice rolls, clicks, win/loss jingles)
- Ambient sounds (shop atmosphere)
- Music (if applicable)

### Project Management
- Task tracking and prioritization
- Milestone planning (BASIC4 → FULL44 roadmap)
- Playtesting coordination and feedback collection

### QA & Playtesting
- Bug reporting and tracking
- Balance testing (are parts/synergies fair and fun?)
- UX testing (is the flow intuitive?)
